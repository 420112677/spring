            ...
            <table cellpadding="0" cellspacing="0" class="list_hy">
              ...
              <c:forEach items="${LIST}" var="staff">
		              <tr>
		                <td>${staff.name}</td>
		                <td>${staff.sex }</td>
		                <td><fmt:formatDate value="${staff.bornDate}" pattern="yyyy年MM月dd日"/></td>
		                <td><fmt:formatDate value="${staff.workTime}" pattern="yyyy年MM月dd日 HH:mm:ss"/></td>
		                <td>${staff.department.name}</td>
		                <td>${staff.status }</td>
		                <td>
							<a href="toEdit.do?id=${staff.id }" class="btn">编辑</a>
							<a href="delete.do?id=${staff.id }" class="btn">删除</a>
							<a href="toDetail.do?id=${staff.id }" class="btn">详情</a>
						</td>
		              </tr>
               </c:forEach>
            </table>
